...
GPIO_TypeDef* COM_TX_PORT[COMn] = {EVAL_COM1_TX_GPIO_PORT,EVAL_COM2_TX_GPIO_PORT,EVAL_COM3_TX_GPIO_PORT,EVAL_COM4_TX_GPIO_PORT,EVAL_COM5_TX_GPIO_PORT,EVAL_COM6_TX_GPIO_PORT};
 
GPIO_TypeDef* COM_RX_PORT[COMn] = {EVAL_COM1_RX_GPIO_PORT,EVAL_COM2_RX_GPIO_PORT,EVAL_COM3_RX_GPIO_PORT,EVAL_COM4_RX_GPIO_PORT,EVAL_COM5_RX_GPIO_PORT,EVAL_COM6_RX_GPIO_PORT};

const uint32_t COM_USART_CLK[COMn] = {EVAL_COM1_CLK,EVAL_COM2_CLK,EVAL_COM3_CLK,EVAL_COM4_CLK,EVAL_COM5_CLK,EVAL_COM6_CLK};

const uint32_t COM_TX_PORT_CLK[COMn] = {EVAL_COM1_TX_GPIO_CLK,EVAL_COM2_TX_GPIO_CLK,EVAL_COM3_TX_GPIO_CLK,EVAL_COM4_TX_GPIO_CLK,EVAL_COM5_TX_GPIO_CLK,EVAL_COM6_TX_GPIO_CLK};
 
const uint32_t COM_RX_PORT_CLK[COMn] = {EVAL_COM1_RX_GPIO_CLK,EVAL_COM2_RX_GPIO_CLK,EVAL_COM3_RX_GPIO_CLK,EVAL_COM4_RX_GPIO_CLK,EVAL_COM5_RX_GPIO_CLK,EVAL_COM6_RX_GPIO_CLK};

const uint16_t COM_TX_PIN[COMn] = {EVAL_COM1_TX_PIN,EVAL_COM2_TX_PIN,EVAL_COM3_TX_PIN,EVAL_COM4_TX_PIN,EVAL_COM5_TX_PIN,EVAL_COM6_TX_PIN};

const uint16_t COM_RX_PIN[COMn] = {EVAL_COM1_RX_PIN,EVAL_COM2_RX_PIN,EVAL_COM3_RX_PIN,EVAL_COM4_RX_PIN,EVAL_COM5_RX_PIN,EVAL_COM6_RX_PIN};
 
const uint8_t COM_TX_PIN_SOURCE[COMn] = {EVAL_COM1_TX_SOURCE,EVAL_COM2_TX_SOURCE,EVAL_COM3_TX_SOURCE,EVAL_COM4_TX_SOURCE,EVAL_COM5_TX_SOURCE,EVAL_COM6_TX_SOURCE};

const uint8_t COM_RX_PIN_SOURCE[COMn] = {EVAL_COM1_RX_SOURCE,EVAL_COM2_RX_SOURCE,EVAL_COM3_RX_SOURCE,EVAL_COM4_RX_SOURCE,EVAL_COM5_RX_SOURCE,EVAL_COM6_RX_SOURCE};
 
const uint8_t COM_TX_AF[COMn] = {EVAL_COM1_TX_AF,EVAL_COM2_TX_AF,EVAL_COM3_TX_AF,EVAL_COM4_TX_AF,EVAL_COM5_TX_AF,EVAL_COM6_TX_AF};
 
const uint8_t COM_RX_AF[COMn] = {EVAL_COM1_RX_AF,EVAL_COM2_RX_AF,EVAL_COM3_RX_AF,EVAL_COM4_RX_AF,EVAL_COM5_RX_AF,EVAL_COM6_RX_AF};
...
void STM_EVAL_LED_ALL_FIFO(int delay ,int num)
{
    int i=0;
	for(i=0;i<num;i++)
	{
...
void STM_EVAL_LED_FIFO(int delay)
{
	static int index=0;
	STM_EVAL_LEDOn(index); 
	vTaskDelay(delay);
	STM_EVAL_LEDOff(index); 
	vTaskDelay(delay);
	index++;
	if(index>=BLUE) index= 0;
}
void STM_EVAL_LED_FIFO_2(void )
{
	static int index=0;	
	static int offflag=0;
	
	if(offflag)STM_EVAL_LEDOn(index); 
	else      STM_EVAL_LEDOff(index); 
	
	index++;
	 
	
	if(offflag==1) offflag= 0;
	else if(offflag==0) offflag= 1;
...
void STM_EVAL_OBD_COMInit(COM_TypeDef COM, USART_InitTypeDef* USART_InitStruct)
{
  GPIO_InitTypeDef GPIO_InitStructure;
...
void OBD_EVAL_COMInit( uint32_t baudrate,uint32_t parity)
{
	...
	STM_EVAL_OBD_COMInit(COM5,&USART_InitStructure);
 	STM_EVAL_OBD_COMInit(COM6,&USART_InitStructure);
	
	USART_ITConfig(COM_USART[COM5], USART_IT_RXNE, ENABLE);
	USART_ITConfig(COM_USART[COM6], USART_IT_RXNE, ENABLE);
	USART_ClearFlag(COM_USART[COM5], USART_IT_RXNE) ;
	USART_ClearFlag(COM_USART[COM6], USART_IT_RXNE) ;
...
void OBD_EVAL_GPIOInit(void)
{
	GPIO_InitTypeDef GPIO_InitStructure;

	/* Enable GPIO clock */
	RCC_AHB1PeriphClockCmd(COM_TX_PORT_CLK[COM5] , ENABLE);
	RCC_AHB1PeriphClockCmd(COM_TX_PORT_CLK[COM6] , ENABLE);	
	...
	GPIO_InitStructure.GPIO_Pin = TX00_CTL|TX01_CTL|TX10_CTL|TX11_CTL ;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(TX_CTL_PORT, &GPIO_InitStructure);


	GPIO_InitStructure.GPIO_Pin = J1850_RX0_PIN ;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
   	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
	GPIO_Init(J1850_RX0_PORT, &GPIO_InitStructure);



	GPIO_InitStructure.GPIO_Pin = J1850_RX1_PIN ;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
   	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
	GPIO_Init(J1850_RX1_PORT, &GPIO_InitStructure);
