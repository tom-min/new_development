...
//http://blog.sina.com.cn/s/blog_4ab2ba570100iria.html
void main_xml2(void)
{
	mxml_node_t *xml;    
	...
	mxmlNewText(node, 0, "1231569");


	rts = f_open(&fp,"filename.xml", FA_OPEN_ALWAYS| FA_WRITE | FA_READ);
	printfuartdebug("filename f_open==%d\r\n",rts );
	

	
	ret=mxmlSaveFile(xml, &fp,whitespace_cb );

	rts = f_sync(&fp);   
	f_close(&fp);
       mxmlDelete(xml);
	rts = f_open(&fp,"filename.xml", FA_OPEN_EXISTING|FA_READ);

	for(i=0;i<f_size(&fp);)
	{
		rts =  f_read(&fp,Readbuffer,sizeof(Readbuffer),&SizeReaded);
		i+=SizeReaded;
		printfuartdebug("%s",Readbuffer );
...
int					/* O - Exit status */
main_xml(int  argc,				/* I - Number of command-line args */
     char *argv[])			/* I - Command-line args */
{
  int			i;		/* Looping var */
  ...
  */
  ...
  if (argc != 2 && argc != 3)
  {
  //  printfuartdebug("ERROR: No parent node in basic test.\n" );
  //  return (1);
  }
  ...
  */
  ...
  tree = mxmlNewElement(MXML_NO_PARENT, "element");
	...
	{
	   printfuartdebug("ok: mxmlNewElement.\n");
	}

  if (tree->type != MXML_ELEMENT)
  {
    printfuartdebug("ERROR: Parent has type %s (%d), expected MXML_ELEMENT.\n",
            tree->type < MXML_ELEMENT || tree->type > MXML_TEXT ?
	        "UNKNOWN" : types[tree->type], tree->type);
    mxmlDelete(tree);
    return (1);
  ...
  if (strcmp(tree->value.element.name, "element"))
  {
    printfuartdebug( "ERROR: Parent value is \"%s\", expected \"element\".\n",
            tree->value.element.name);
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!node)
  {
    printfuartdebug("ERROR: No first child node in basic test.\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (node->type != MXML_INTEGER)
  {
    printfuartdebug( "ERROR: First child has type %s (%d), expected MXML_INTEGER.\n",
            node->type < MXML_ELEMENT || node->type > MXML_TEXT ?
	        "UNKNOWN" : types[node->type], node->type);
    mxmlDelete(tree);
    return (1);
  ...
  if (node->value.integer != 123)
  {
    printfuartdebug("ERROR: First child value is %d, expected 123.\n",
            node->value.integer);
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!node)
  {
    printfuartdebug("ERROR: No second child node in basic test.\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (node->type != MXML_OPAQUE)
  {
    printfuartdebug("ERROR: Second child has type %s (%d), expected MXML_OPAQUE.\n",
            node->type < MXML_ELEMENT || node->type > MXML_TEXT ?
	        "UNKNOWN" : types[node->type], node->type);
    mxmlDelete(tree);
    return (1);
  ...
  if (!node->value.opaque || strcmp(node->value.opaque, "opaque"))
  {
    printfuartdebug( "ERROR: Second child value is \"%s\", expected \"opaque\".\n",
            node->value.opaque ? node->value.opaque : "(null)");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!node)
  {
    printfuartdebug("ERROR: No third child node in basic test.\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (node->type != MXML_REAL)
  {
    printfuartdebug("ERROR: Third child has type %s (%d), expected MXML_REAL.\n",
            node->type < MXML_ELEMENT || node->type > MXML_TEXT ?
	        "UNKNOWN" : types[node->type], node->type);
    mxmlDelete(tree);
    return (1);
  ...
  if (node->value.real != 123.4f)
  {
    printfuartdebug("ERROR: Third child value is %f, expected 123.4.\n",
            node->value.real);
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!node)
  {
    printfuartdebug("ERROR: No fourth child node in basic test.\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (node->type != MXML_TEXT)
  {
    printfuartdebug("ERROR: Fourth child has type %s (%d), expected MXML_TEXT.\n",
            node->type < MXML_ELEMENT || node->type > MXML_TEXT ?
	        "UNKNOWN" : types[node->type], node->type);
    mxmlDelete(tree);
    return (1);
  ...
  if (!node->value.text.whitespace ||
      !node->value.text.string || strcmp(node->value.text.string, "text"))
  {
    printfuartdebug("ERROR: Fourth child value is %d,\"%s\", expected 1,\"text\".\n",
            node->value.text.whitespace,
	    node->value.text.string ? node->value.text.string : "(null)");
    mxmlDelete(tree);
    return (1);
  ...
  for (i = 0; i < 4; i ++)
  {
    ...
    if (!node)
    {
      printfuartdebug( "ERROR: No group #%d child node in basic test.\n", i + 1);
      mxmlDelete(tree);
      return (1);
    }
    ...
    if (node->type != MXML_ELEMENT)
    {
      printfuartdebug( "ERROR: Group child #%d has type %s (%d), expected MXML_ELEMENT.\n",
              i + 1, node->type < MXML_ELEMENT || node->type > MXML_TEXT ?
	                 "UNKNOWN" : types[node->type], node->type);
      mxmlDelete(tree);
      return (1);
  ...
  */
  ...
  if (!node)
  {
    printfuartdebug("ERROR: Unable to find value for \"*/two\".\n");
    mxmlDelete(tree);
    return (1);
  }
  else if (node->type != MXML_OPAQUE || strcmp(node->value.opaque, "value"))
  {
    printfuartdebug("ERROR: Bad value for \"*/two\".\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!node)
  {
    printfuartdebug("ERROR: Unable to find value for \"foo/*/two\".\n");
    mxmlDelete(tree);
    return (1);
  }
  else if (node->type != MXML_OPAQUE || strcmp(node->value.opaque, "value"))
  {
    printfuartdebug("ERROR: Bad value for \"foo/*/two\".\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!node)
  {
    printfuartdebug("ERROR: Unable to find value for \"foo/bar/one/two\".\n");
    mxmlDelete(tree);
    return (1);
  }
  else if (node->type != MXML_OPAQUE || strcmp(node->value.opaque, "value"))
  {
    printfuartdebug("ERROR: Bad value for \"foo/bar/one/two\".\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  */
  ...
  if (!ind)
  {
    printfuartdebug("ERROR: Unable to create index of all nodes.\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (ind->num_nodes != 13)
  {
    printfuartdebug(stderr, "ERROR: Index of all nodes contains %d "
                    "nodes; expected 13.\n", ind->num_nodes);
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
    return (1);
  ...
  if (!mxmlIndexFind(ind, "group", NULL))
  {
    printfuartdebug("ERROR: mxmlIndexFind for \"group\" failed.\n");
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!ind)
  {
    printfuartdebug("ERROR: Unable to create index of groups.\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (ind->num_nodes != 4)
  {
    printfuartdebug("ERROR: Index of groups contains %d "
                    "nodes; expected 4.\n", ind->num_nodes);
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
    return (1);
  ...
  if (!mxmlIndexEnum(ind))
  {
    printfuartdebug("ERROR: mxmlIndexEnum failed.\n");
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!ind)
  {
    printfuartdebug("ERROR: Unable to create index of type attributes.\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (ind->num_nodes != 3)
  {
    printfuartdebug("ERROR: Index of type attributes contains %d "
                    "nodes; expected 3.\n", ind->num_nodes);
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
    return (1);
  ...
  if (!mxmlIndexFind(ind, NULL, "string"))
  {
    printfuartdebug("ERROR: mxmlIndexFind for \"string\" failed.\n");
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (!ind)
  {
    printfuartdebug("ERROR: Unable to create index of elements and attributes.\n");
    mxmlDelete(tree);
    return (1);
  }
  ...
  if (ind->num_nodes != 3)
  {
    printfuartdebug(stderr, "ERROR: Index of elements and attributes contains %d "
                    "nodes; expected 3.\n", ind->num_nodes);
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
    return (1);
  ...
  */
  ...
  for (i = 0; i < 12; i ++)
  {
    ...
    {
      printfuartdebug( "ERROR: Child pointer prematurely NULL on child #%d\n",
              i + 1);
      mxmlDelete(tree);
      return (1);
    }
  ...
  if (tree->child)
  {
    printfuartdebug("ERROR: Child pointer not NULL after deleting all children.\n");
    return (1);
  }

  if (tree->last_child)
  {
    printfuartdebug("ERROR: Last child pointer not NULL after deleting all children.\n");
    return (1);
  }
  ...
  */
  ...
  if (!tree)
  {
    printfuartdebug("Unable to read XML file with default callback.\n", stderr);
    return (1);
  }
  ...
  if (!strcmp(argv[1], "test.xml"))
  {
    ...
    */
    ...
    if ((node = mxmlFindPath(tree, "group/option/keyword")) == NULL)
    {
      printfuartdebug("Unable to find group/option/keyword element in XML tree.\n");
      mxmlDelete(tree);
      return (1);
    }
    ...
    if (node->type != MXML_TEXT)
    {
      printfuartdebug("No child node of group/option/keyword.\n");
    ...
    if ((text = mxmlGetText(node, NULL)) == NULL || strcmp(text, "InputSlot"))
    {
      printfuartdebug( "Child node of group/option/value has value \"%s\" instead of \"InputSlot\".\n", text ? text : "(null)");
      mxmlDelete(tree);
      return (1);
    }
  }
  ...
  */
  ...
  if (!tree)
  {
    printfuartdebug("Unable to read XML file.\n", stderr);
    return (1);
  }
  ...
  if (!strcmp(argv[1], "test.xml"))
  {
    ...
    */
    ...
    if ((node = mxmlFindElement(tree, tree, "choice", NULL, NULL,
                                MXML_DESCEND)) == NULL)
    {
      printfuartdebug("Unable to find first <choice> element in XML tree.\n");
      mxmlDelete(tree);
      return (1);
    }
    ...
    if (!mxmlFindElement(node, tree, "choice", NULL, NULL, MXML_NO_DESCEND))
    {
      printfuartdebug("Unable to find second <choice> element in XML tree.\n");
      mxmlDelete(tree);
      return (1);
    }
  }
